...
void initializeOrtSessionBundle(RuntimeLibraryHandle runtimeDll, const std::string& modelPath, const char* logId, OrtSessionBundle& bundle) {
...
public:
    explicit YoloPoseOnnxExtractor(const std::string& modelPath) {
        ...
        initializeOrtSessionBundle(runtimeDll_, modelPath, "fight_detection_pose", session_);
    }
...
cv::VideoCapture openCapture(const Config& config, std::string& sourceDesc) {
    if (config.sourceType == "camera") {
        sourceDesc = "camera:" + std::to_string(config.cameraIndex);
#ifdef _WIN32
        cv::VideoCapture cap(config.cameraIndex, cv::CAP_DSHOW);
#else
        cv::VideoCapture cap(config.cameraIndex, cv::CAP_ANY);
#endif
        if (!cap.isOpened()) {
...
cv::VideoWriter buildWriter(const Config& config, const cv::VideoCapture& cap, const cv::Mat& frame) {
...
OnnxRuntimeClassifier::OnnxRuntimeClassifier(
    ...
    if (onnxEp_ == "cann") {
        throw std::runtime_error(
            "onnx-ep=cann requested, but current binary is built with CPU-only ONNX Runtime package. "
            "Please rebuild ONNX Runtime with CANN Execution Provider and ensure CANN runtime is installed.");
    }
    ...
    initializeOrtSessionBundle(impl_->runtimeDll, modelPath_, "fight_detection_demo", impl_->session);
#endif
}
...
Config parseArgs(int argc, char** argv) {
    ...
    if (auto value = getArgValue(args, "--camera-index")) {
        config.cameraIndex = std::stoi(*value);
    }
...
Summary runDemo(const Config& config) {
    ...
    cv::VideoCapture cap = openCapture(config, sourceDesc);
    if (!cap.isOpened()) {
    ...
    if (config.featureSource == "yolo-onnx") {
        poseExtractor = std::make_unique<YoloPoseOnnxExtractor>(config.poseModelPath);
    }
#else
    if (config.featureSource == "yolo-onnx") {
        throw std::runtime_error(
